...
-- Fixes Assets Manager sign-out scope to use the asset's current location.
...
begin
  ...
  select id into v_traveling_location_id
  ...
  insert into public.signouts (signed_out_by, signed_out_to, to_department_id, package_name, notes, expected_return)
  values (v_actor, v_recipient, v_traveling_location_id, package_name, nullif(trim(notes), ''), now())
  returning id into v_signout_id;

  foreach v_asset_id in array target_asset_ids loop
    select id, code, status, department_id, current_location_id, locked_by, locked_at
    into v_asset
    from public.assets
    where id = v_asset_id
    for update;
    ...
    values (
      ...
      v_recipient,
      case
        when history_notes_by_asset is not null and history_notes_by_asset ? v_asset_id::text then history_notes_by_asset ->> v_asset_id::text
        ...
        else coalesce(nullif(trim(notes), ''), 'Location moved to Traveling.')
      end
    );
  ...
  return jsonb_build_object('success', true, 'signout_id', v_signout_id, 'processed_count', v_count);
end;
$$;
