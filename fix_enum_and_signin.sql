-- ============================================================
-- FIX: Rename legacy enum values + re-apply sign-in RPC
-- Root cause: migration 20260423 (enum renames) was never
-- applied to the live Supabase project.
-- ============================================================

-- 1. Rename legacy enum values to match what the app expects
...
-- 2. Add not_assigned if missing (from migration 20260425)
ALTER TYPE public.asset_status ADD VALUE IF NOT EXISTS 'not_assigned';

-- 3. Re-apply the sign_in_assets_safe function with normalization
...
begin
  ...
  for v_entry in select value from jsonb_array_elements(signin_payload) loop
    ...
    select id, code, status, department_id, current_holder
    into v_asset
    from public.assets
    where id = v_asset_id
    for update;
